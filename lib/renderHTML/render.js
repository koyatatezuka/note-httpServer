...
render.todoHtml = (dirHtmlPath, dirDataPath, req, res) => {
	fs.readFile(dirHtmlPath, (err, data) => {
		...
		notes.forEach(note => {
			...
			<textarea name="note">${note.note}</textarea><input type="hidden" name="id" value="${note.id}"/><div class="btn-container"><button class="delete-btn" type="submit" name="method" value="delete" value="DELETE"><i class="fas fa-minus-circle"></i></button>
			<button class="update-btn" type="submit" name="method" value="put"><i class="fas fa-plus-circle"></i></button></div></form>`;
		});
